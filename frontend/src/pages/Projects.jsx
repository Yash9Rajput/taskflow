...
/* ─── Inline Project Form (like NoteForm in Notes.jsx) ─── */
function ProjectForm({ initial, users, onSave, onCancel }) {
  ...
  const filteredUsers = users.filter(u =>
    !memberSearch || u.name?.toLowerCase().includes(memberSearch.toLowerCase()) ||
  ...
  const handleSave = async () => {
    ...
    try {
      if (initial) {
        await projectsAPI.update(initial.id, { name: form.name, description: form.description, member_ids: form.member_ids });
      } else {
        await projectsAPI.create({ name: form.name, description: form.description, member_ids: form.member_ids });
      }
      onSave();
    } catch (e) {
      setError(e?.response?.data?.message || 'Failed to save project. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ animation: 'scaleIn 0.3s ease', overflowY: 'auto', maxHeight: 'calc(100vh - 140px)' }}>
      ...
      <div className="card" style={{ padding: '2rem', maxWidth: 720, margin: '0 auto' }}>
        {error && (
          <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: 8, padding: '0.75rem 1rem', marginBottom: '1rem', color: '#f87171', fontSize: 13 }}>
            ⚠ {error}
          </div>
        )}
            ...
            {filteredUsers.map(u => {
              ...
              return (
                <div key={u.id}
                  ...
                  {sel && <span style={{ color: '#818cf8', fontSize: 16 }}>✓</span>}
                </div>
              );
            })}
          </div>
...
export default function Projects() {
  ...
  /* ── Inline form views (no modal, no footer overlap) ── */
  if (view === 'new') return (
    <ProjectForm
      users={users}
    ...
    <div style={{ overflowY: 'auto' }}>
      <div className="section-head au">
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          {isAdmin && <button className="btn btn-primary" onClick={() => setView('new')}>+ New Project</button>}
        </div>
      </div>
            ...
            <div key={p.id} className={`card card-hover ${isSel ? 'card-selected' : ''}`}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                  {isAdmin && <button className="btn btn-sm" onClick={() => { setEditProj(p); setView('edit'); }}>✎</button>}
            ...
            <div style={{ marginBottom: '1.5rem', padding: '1rem', background: 'rgba(99,102,241,0.05)', borderRadius: 'var(--r-md)', border: '1px solid rgba(99,102,241,0.12)' }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                {selProject.members.map(m => {
                  const memberTasks = projTasks.filter(t => t.assignee_id === m.id);
                  ...
                  return (
                    <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'rgba(255,255,255,0.04)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border)' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--text-3)' }}>{memberTasks.length} tasks · {memberDone} done</div>
                      </div>
                      <Badge role={m.role} />
                    </div>
                  );
        ...
        <div className="modal-overlay" onClick={() => setDeleteConfirm(null)}>
          <div className="modal-box" style={{ maxWidth: 400, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: '1.5rem', lineHeight: 1.6 }}>
              {deleteConfirm.type === 'project' ? 'This will permanently delete the project and all its tasks.' : 'This will permanently delete this task.'}
            </div>

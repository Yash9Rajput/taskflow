...
export default function Login() {
  ...
  // Check for errors from Google OAuth redirect (e.g. ?error=google_cancelled)
  React.useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const err = params.get('error');
    if (err === 'google_cancelled') setError('Google sign-in was cancelled.');
    else if (err === 'google_token_failed') setError('Google sign-in failed. Please try again.');
    else if (err === 'google_no_email') setError('Could not get your email from Google. Please use email/password.');
    else if (err === 'google_server_error') setError('A server error occurred. Please try again.');
    // Clean up the URL
    ...
    <div className="auth-wrap">
      ...
      <div className="auth-card au">
        ...
        <div className="auth-box">
          ...
          {/* Google button — now a real redirect */}
          <button
            onClick={handleGoogle}
            style={{width:'100%',padding:'11px 16px',borderRadius:'var(--r-sm)',border:'1px solid var(--border-hi)',background:'rgba(255,255,255,0.05)',color:'var(--text)',fontFamily:'var(--font-b)',fontSize:14,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10,marginBottom:'1.25rem',transition:'all 0.2s'}}
          ...
          <form onSubmit={handleSubmit} autoComplete="on">
            ...
            <div className="field">
              <label style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <Link
                  to="/forgot-password"
                  style={{fontSize:12,color:'#a5b4fc',fontWeight:500,textDecoration:'none'}}

...
export default function Team() {
  ...
  // Group users: admins first, then members
  ...
  const renderUserCard = (u) => {
    ...
    const isDevUser = DEV_EMAILS.includes(u.email);
      ...
      <div key={u.id} className="card" style={{padding:'1.25rem',cursor:'pointer'}}
        ...
        {expanded===u.id && uTasks.length>0&&(
          <div style={{marginTop:'1rem',paddingTop:'1rem',borderTop:'1px solid var(--border)'}}>
            <div style={{fontSize:11,fontWeight:600,color:'var(--text-3)',marginBottom:8,textTransform:'uppercase',letterSpacing:'0.05em'}}>Assigned Tasks</div>
            {uTasks.slice(0,8).map(t=>(
    ...
    <div style={{overflowY:'auto'}}>
      ...
      {filterUsers(admins).length > 0 && (
        <div className="au2" style={{marginBottom:'2rem'}}>
          ...
          <div className="grid-2">{filterUsers(admins).map(renderUserCard)}</div>
        </div>
      )}
      ...
      {filterUsers(members).length > 0 && (
        <div className="au3">
          ...
          <div className="grid-2">{filterUsers(members).map(renderUserCard)}</div>
        </div>
      )}

      {filterUsers(users).length === 0 && (
        <Empty message={search ? "No members match your search." : "No team members yet."}/>
      )}
        ...
        <div className="modal-overlay" onClick={()=>setDeleteConfirm(null)}>
          <div className="modal-box" style={{maxWidth:400,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:13,color:'var(--text-3)',marginBottom:'1.5rem',lineHeight:1.6}}>
              Are you sure? Their tasks will remain but they will lose access.
            </div>

...
router.post('/',
  ...
  body('project_id').optional({ nullable: true }).isString(),   // ← was required, now optional
  body('description').optional({ nullable: true }).trim(),
  body('status').optional().isIn(['todo', 'in-progress', 'done']),
  body('priority').optional().isIn(['low', 'medium', 'high', 'urgent']),  // ← added urgent
  ...
  async (req, res) => {
    try {
      ...
      // Only validate project if one was provided
      if (project_id) {
